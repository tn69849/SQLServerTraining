...
-- Edit the following a query to return data from the system_health session, using the DMF 
...
xe_event.c.query('') AS deadlock_data
FROM
(
	SELECT CAST(event_data AS xml) AS xe_data
...
WHERE xe_event.c.value('@name', 'varchar(100)') = ''
;
